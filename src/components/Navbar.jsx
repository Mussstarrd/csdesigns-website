...
const linkClass = ({ isActive }) =>
  `font-mono text-xs tracking-[0.2em] uppercase transition-all ${
    isActive
      ? "text-cs-cyan glow-cyan"
      : "text-cs-muted hover:text-cs-cyan"
  }`;
    ...
    <header className="sticky top-0 z-50 backdrop-blur-md bg-cs-bg/80 border-b border-cs-line">
      <div className="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-3 group">
          <Logo size={28} />
          <span className="font-display text-lg font-bold tracking-widest text-cs-ink group-hover:text-cs-cyan transition-colors">
            CSDESIGNS
          </span>
        </Link>
